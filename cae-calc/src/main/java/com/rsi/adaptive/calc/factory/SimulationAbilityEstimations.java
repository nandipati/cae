...
import com.rsi.adaptive.calc.util.Constants;
...
public class SimulationAbilityEstimations implements AbilityEstimations {
  ...
  public EstimationsDomain getEstimations(List<CurrentItemsDomain> currentItemsList,
    ...
    Optional<ScoreEstimationRequestDomain> scoreEstimations = customStateRequestDomain.getScoreEstimation().stream().findFirst();

    if(scoreEstimations.isPresent()){
      ScoreEstimationRequestDomain estimationDomain = scoreEstimations.get();
    ...
    if (!CollectionUtils.isEmpty(currentItemsList)) {
      ...
      if(currentItemsList.size() >=Constants.ROUTING_TEST){
        // after 5th item
        ...
        long correctCount = myResponseList.stream().filter(response -> response.equals(Constants.CORRECT)).count();
        long inCorrectCount = myResponseList.stream().filter(response -> response.equals(Constants.IN_CORRECT)).count();
